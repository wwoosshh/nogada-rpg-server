...
describe('gatherToolProfile', () => {
  ...
  // 왜: 설계 §2-2 가 파는 것은 "분당 산출 +33%" 인데, **이 테스트가 재는 것은
  //     산출이 아니라 분당 채집 횟수다.** 둘은 같은 숫자가 아니다: 4단은 간격만
  //     당기는 것이 아니라 roll 배수도 0.8 → 0.7 로 낮춰 질량을 희귀 쪽으로
  //     옮기므로(gatherToolProfile), 실제 산출은 +33% **보다 더** 오른다.
  //     그러니 이 줄이 증명하는 것은 그 문장의 **하한**이다.
  //     골드로 실측하는 것은 4단 아이템이 서는 태스크의 몫이다(`goldPerMinute`) —
  //     굴릴 표도 값을 매길 산출물도 그때 생긴다.
  //
