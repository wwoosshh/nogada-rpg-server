...
describe('claimPlausible — 속도 개연성(§2-3), 거리는 맨해튼이다', () => {
  const last = { x: 0, y: 0, atMs: 100_000 }

  it('직전 주장이 없으면(첫 주장·전환 직후) 공회전한다 — 의도다(§12-앞 7)', () => {
    expect(claimPlausible(null, { x: 99, y: 99 }, 0)).toBe(true)
  })

  it('정직한 걸음 속도의 주장은 통과한다', () => {
    // 2,500ms 에 맨해튼 10칸: 예산 12.5 + 여유 ≥ 10.
    expect(claimPlausible(last, { x: 5, y: 5 }, 100_000 + 10 * STEP_MS + 500)).toBe(true)
  })
  ...
  it('대각 순간이동은 거절한다 — 체비쇼프 자였다면 통과했을 주장이다', () => {
    ...
    expect(claimPlausible(last, { x: 5, y: 5 }, now)).toBe(false)
  })

  it('같은 거리라도 시간이 넉넉하면 통과한다 — 자르는 것은 거리가 아니라 속도다', () => {
    expect(claimPlausible(last, { x: 5, y: 5 }, 100_000 + 10 * STEP_MS)).toBe(true)
  })
})
