...
export const PlayerStateSchema = z.object({
  ...
  // gold 와 **정확히 같은 이유로** 기본값을 단다: 이 둘은 스토리 아크에서 생긴
  // 필드이므로 그 전에 저장된 플레이어에게는 키가 통째로 없고, 필수로 두면
  // readPlayers(store.ts)가 그 플레이어를 통째로 버린다 — 숙련도도 인벤토리도
  // 강화한 도구도 같이. 사슬을 한 번도 못 본 세이브는 "첫 마디에 서 있다"와 같은
  // 뜻이라 마이그레이션 없이 0 이 맞는 답이다(친구들 계정이 이미 살아 있다).
  //
  // 그 사람들이 초보 안내를 받지 않는 것은 이 기본값이 아니라 story.csv 의
  // `catchUp` 이 진다 — 첫 판정 훅이 돌 때 이미 지나친 마디를 밀어 올린다(설계 ⑦).
  //
  // **여기 기본값은 리터럴이어도 된다.** dialogueHistory·donated 가 함수를 쓰는
  // 이유(참조형 리터럴을 주면 키 없는 세이브들이 zod 가 만든 **같은 객체 하나**를
  // 공유해서, 한 사람의 진행이 다른 사람에게도 보인다)가 숫자에는 없다 — 값형은
  // 대입할 때마다 복사되므로 나눠 가질 객체 자체가 없다. weather 의 `.default(null)`
  // 이 같은 자리에 서 있다.
  //
  // `.int().min(0)` 인 이유는 stacks·gold 와 같다: 마디 번호도 델타도 더하기로만
  // 움직이므로 음수나 소수는 어떤 경로로도 생기지 않는다. 상한을 안 거는 이유는
  // hp 와 같다 — 마디 수는 CSV 가 정하는 게임 값이고, 그 수를 여기 박아 두면
  // 사슬을 늘리는 날 이미 끝낸 사람의 세이브가 통째로 거절된다.
  story: z.number().int().min(0).default(0),
  storyCount: z.number().int().min(0).default(0),
  // 위 둘과 **같은 이유로** 기본값을 단다 — 아크 F 가 늘린 셋째 칸이라, 그 전에
  // 저장된 플레이어에게는 키가 통째로 없다.
  //
  // **기본값이 `''` 인 것이 요점이다.** 여기 '눈의마을' 이라고 적으면 이 문이
  // 시작 맵을 아는 두 번째 자리가 되고(`defaultLocation` 이 빈 맵 id 를 쓰는 그
  // 이유), 그보다 나쁜 것은 **키 없는 세이브 전부가 눈의마을 사람이 된다**는
  // 쪽이다 — 이 필드가 고치려고 생긴 사고가 정확히 그것이다. 빈 문자열은
  // maps.csv 가 만들 수 없는 id 라(requireCell 이 빈 칸을 거절한다) 어떤 마을과도
  // 안 겹치고, "아직 모른다" 를 뜻한다. 채우는 것은 첫 판정 훅이다
  // (`pinStartVillage` — 유도에 근거가 있을 때만 적는다).
  //
  // `.min(1)` 을 안 거는 이유도 `PlayerLocationSchema.mapId` 와 같다: 그 빈 값이
  // 자리표시자 그 자체다. 실재하는 마을인지는 여기서 안 본다 — 마을 목록은
  // packages/data 의 것이고(shared 는 data 를 못 본다), 마을을 고쳐 그린 날 이
  // 문이 옛 목록으로 남으면 세이브가 통째로 거절된다.
  //
  // 기본값이 리터럴이어도 되는 이유는 story 와 같다(문자열도 값형이다).
  startVillage: z.string().default(''),
