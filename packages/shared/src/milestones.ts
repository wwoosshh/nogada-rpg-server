 ...
 *
 * **지금 이 함수를 부르는 곳은 아래 자기 재귀와 이 패키지의 검사뿐이다** — 화면도
 * 서버도 안 쓴다. 그래도 지우지 않는 이유는 아래 `every` 규칙이다: 합산 이정표가
 * "얼마나 왔는가"에 답하는 유일한 방법이고, 그 답은 개수가 아니라 병목이다.
 * **다시 정렬에 끌어다 쓰지 마라** — 신규는 40개가 전부 0.000 이라 그 정렬은
 * 아무것도 안 가르고, 그것이 이 아크가 고친 버그 그 자체다.
 *
