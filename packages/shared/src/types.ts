...
export interface PlayerState {
  ...
  storyCount: number
  /**
   * 이 사람이 **캐릭터를 만들 때 고른 마을**의 맵 id. 사슬 한 벌을 그 사람의
   * 것으로 펴는 슬롯(`{마을}`·`{채집장}`·`{계열}`·`{문방향}`)이 전부 여기서 나온다.
   *
   * **왜 저장하는가 — 유도로는 원리적으로 복원이 안 되는 사실이다.** `story` 와
   * 같은 종류이지만 이유가 한 겹 더 나쁘다: 숙련이 전부 0 이고(아직 아무것도 안
   * 캤다) 마을·채집장 밖에 서 있는 사람에게는 세계 어디에도 "이 사람이 어느
   * 마을에서 났는가" 를 말해 주는 값이 없다. 그런데 그 상태가 드물지 않다 —
   * 마을에서 월드맵으로 한 칸 나가면 그것이다. 그때 유도는 늘 같은 답(전환표
   * 첫 마을)을 냈고, 띠와 깃발은 **그 답을 말한다**: 북동쪽마을을 고른 사람이
   * 「눈의 마을 북문으로 나가라」를 읽고, 따라가서 얼음을 한 번 캐는 순간 숙련
   * 유도가 그 답을 영구히 못박았다. 이 아크가 고치겠다고 한 상태(새 계정 넷 중
   * 셋이 남의 안내를 받는다)와 같은 모양이다.
   *
   * 값은 **캐릭터를 만들 때** 적는다(`createInitialPlayer`) — 그 순간 이미 아는
   * 값이라 유도할 일이 없다. 빈 문자열은 "모른다" 는 뜻이고, 이 필드가 생기기
   * 전의 세이브(친구들 계정)가 그 상태다. 그 사람들은 첫 판정 훅에서 한 번
   * 유도되어 채워진다(`pinStartVillage`).
   *
   * 기본값이 리터럴이어도 되는 이유는 `story` 와 같다 — 문자열도 값형이라
   * 세이브 여럿이 나눠 가질 객체가 없다(protocol.ts:137-140 의 규범은 참조형에
   * 걸린다).
   */
  startVillage: string
  /**
...
export interface NodeDef {
   ...
   * 열릴 때만 서는 옆길(`special`)인가. **화면은 이 칸을 읽지 않는다**: 마커에
   * 무엇이 그려지는지는 옆의 `sprite` 가 정한다(NodeMarker — 등급이 색을 정하던
   * 시절은 아크 A 가 그림을 달면서 끝났다).
   *
   * **채집 티어 스펙은 이 칸을 "표시 전용"이라 적었고 그것이 대가를 치렀다**
   * (결계 설계 계기 둘): 심층 노드 넷이 이름과 겉모습만 심층인 채 바깥과 같은
   * 표를 굴렸고, 얼음안내판은 그 위에서 거짓말을 했다. 지금은 등급마다 `tableId` 의
   ...
   *
   * **`undefined` 가 "언제나 열림" 이다.** 출하 열두 행 중 보통·심층 여덟이 그쪽이라
