 ...
 *
 * **이 그물은 미루는 쪽만 문다 — 반대 방향에는 아직 자가 없다.** 해금치 **아래**
 * 에서 「이제 내 계산대가 열렸네」라고 말하는 것도 똑같은 거짓말인데 여기서 안
 * 잡는다. 안 세운 것은 잊어서가 아니라 값 때문이다: 개시 표현(「열렸」·「값을 쳐」)
 * 의 낱말 그물은 오탐이 훨씬 흔하고, 이 파일은 오탐 하나를 그것이 막아 준 진짜
 * 오류보다 비싸게 친다(findDeadDialogueRules).
 *
 * 미루는 쪽조차 **참말이 그물 한 칸 아래에 셋 붙어 있다** — 광산노인·숲마을벌목꾼·
 * 항구약초지기의 아래끝 1,000 짜리 자랑 줄이 「이 마을에서 그걸 넘은 사람은 아직
 * 없어」로 참말을 한다. 지금은 1,000 < 해금치 5,000 이라 대상 밖일 뿐이므로,
 * 작가가 그 자랑 줄의 아래끝을 5,000 으로 올리는 날 참말이 빌드를 세운다.
 * 그물을 넓히려는 다음 사람은 그 거리부터 다시 재라.
 */
