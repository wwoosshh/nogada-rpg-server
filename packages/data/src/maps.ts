...
/**
 * **개발 전용 시험장**. 게임의 일부가 아니라 우리가 걸어 보려고 둔 맵이다.
 *
 * 눈의마을 서문에 문이 하나 붙어 있어 **숙련 0 으로 걸어 들어간다.** 그래서 두
 * 검사가 이 목록을 나눠 쓴다:
 *
 * - 주민 경로 그래프에서 뺀다(routeBake) — 넣으면 주민이 시험장을 가로지르는
 *   지름길이 생기고, 그 맵을 고칠 때마다 마을 사람들의 하루가 흔들린다.
 * - **특수 노드를 여기 두지 못하게 막는다**(transitions) — 특수 재료는 유일
 *   출처라, 그 문이 곧 "게임 전체의 그 재료를 숙련 0 에 내준다" 가 된다.
 *
 * 목록이 두 벌이 되면 한쪽만 자란다. 맵 이름을 바꾸면 여기 한 줄만 고친다.
 */
export const DEV_ONLY_MAP_IDS: readonly string[] = ['개발맵']
