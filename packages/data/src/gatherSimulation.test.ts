...
describe('§8-4 네 표 전 티어가 실제로 드랍된다', () => {
  ...
  for (const tableId of Object.keys(tables)) {
    it(`${tableId}: 최종 브라켓에서 사다리의 전 티어가 나오고 실패는 0% 다(§8-3)`, () => {
      ...
      // 이미 선 규범 둘이 강제한 결과다: **도감 형평 하한 하나**가 잡티어의 몫을
      // 대표 표보다 얇게 묶는다(실측: 얼음 ∞ 에서 상한 14,026 = 약 14%).
      // SPECIAL_YIELD_MAX 는 여기서 기여하지 않는다 — ∞ 의 비가 0.004~0.015 다. 둘을 지키면 나머지는 꽝이 될 수밖에 없고,
