...
export function validateGatherTables(tables: GatherTables, data: GameData): GatherTablesCheck {
  ...
  for (const table of Object.values(tables)) {
    ...
    // 선 규범 둘이 **강제한** 결과다: **형평 하한 하나**가 잡티어의 몫을 그 계열
    // 대표 표보다 얇게 묶는다 — 위 SPECIAL_YIELD_MAX 는 여기서 한 톨도 기여하지
    // 않는다(∞ 의 비가 0.004~0.015 라 천장 1.0 에서 65배 여유다)(실측: 얼음 ∞ 에서 상한이 14,026/100,001 = 약 14%). 그 둘을 지키면 나머지는 꽝이 될
