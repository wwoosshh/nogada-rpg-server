...
export function createInitialPlayer(spec: NewCharacterSpec): PlayerState {
  ...
  return {
    ...
    // (`{계열}`·`{채집장}`·`{문방향}`)으로 쓰이고 값은 아래 마을에서 유도된다
    // (storySlots) — 시작 도구를 여기 안 적는 것과 같은 이유다.
    story: 0,
    storyCount: 0,
    // **고른 마을을 그대로 적는다.** 시작 도구와 정반대의 자리다: 도구는 마을에서
    // 유도되지만 마을 자체는 유도할 것이 없다 — 이 사람이 방금 고른 값이고,
    // 여기서 안 적으면 세계 어디에도 남지 않는다. 숙련이 전부 0 인 채로 월드맵에
    // 한 칸 나가는 순간 유도는 전환표 첫 마을(눈의마을)을 내고, 띠와 깃발은 그
    // 답을 말한다 — 북동쪽마을을 고른 사람이 남의 사슬로 걸어 들어간다
    // (PlayerState.startVillage).
