 ...
 * 그 경우에도 정적 서빙은 붙어 있어서, 나중에 생긴 dist 가 재시작 없이 나간다
 * (app.ts 의 serveClient — 그러지 않던 코드가 첫 릴리스를 404 로 만들 뻔했다).
 * 폴더가 있는지 보는 것은 파일시스템의 일이라 여기서는 **경로만 정한다.**
 *
 ...
 *   터널 뒤라면 `127.0.0.1,::1` 이다. 다만 `::1` 은 **지금 안 쓰이는 항목**이다 —
 *   문서화된 바인딩이 전부 IPv4 전용이라(`HOST` 기본 `0.0.0.0`, 런북은
 *   `127.0.0.1`) 소켓 주소가 ::1 이 될 길이 없다. 실측: `0.0.0.0` 소켓에 ::1 로
 *   붙으면 ECONNREFUSED 다. `HOST` 가 `::` 나 `localhost` 가 되는 날을 위한
 *   보험으로 미리 적어 둔 것이다.
