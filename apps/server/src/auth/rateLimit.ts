 ...
 *
 * **무엇이 셈의 단위인가**는 부르는 쪽이 정한다(routes/auth.ts). 이 클래스는
 * `fail` 을 세고 `clear` 로 무를 뿐이고, 무를지 말지가 두 문에서 갈린다:
 *
 * - **로그인은 추측이다.** 성공은 "이 사람이 맞다"라 세어 둔 것을 무를 이유가
 *   된다(`clear`). 무르지 않으면 어제 두어 번 틀린 사람이 오늘 제대로 들어오고도
 *   다음 실패에서 곧장 긴 대기를 받는다 — 막아야 하는 것은 "틀리면서 계속
 *   두드리는 것"이지 자기 집 IP 에서 게임하는 사람이 아니다(가족이 같은 공유기를
 *   쓴다).
 * - **가입은 자원이 생기는 것이다.** 계정 하나가 실제로 만들어졌으므로 성공해도
 *   무르지 않는다(`clear` 를 부르지 않는다). 실측 기준: 한 IP 가 **연달아 6개**
 *   까지 열고 **7번째가** 429 를 받으며, 그 뒤로 5초 대기가 하나 열 때마다
 *   배증해 5분에서 멎는다(SIGNUP_BACKOFF). 상한이 있으니 쫓아내는 것이 아니라
 *   늦추는 것이다. 가입만 세는 표가 따로 있는 이유는 SIGNUP_BACKOFF 의 주석에 있다.
 *
 *   **자유 횟수(5)보다 하나 많은 것이 맞다.** 부르는 쪽이 `retryAfterMs` 를
 *   **먼저** 보고 그 뒤에 `fail` 하기 때문이다(routes/auth.ts) — 여섯 번째 요청은
 *   아직 `blockedUntil` 이 0 인 표를 보고 통과한 다음에야 자기 손으로 5초를
 *   건다. 오래 "5개까지, 6번째부터"라고 적혀 있었는데 실측은
 *   `201 201 201 201 201 201 429 429 429` 였다. 이 문장이 "공개해도 되는가"의
 *   근거로 런북 2장에 실리므로 한 칸이 값비싸다 —
 *   auth.test.ts 의 '가입은 성공해도 세어진다' 가 그 경계를 못 박는다.
 *
 ...
 * 쪽은 결과를 알기 전에 실패로 세어 두고(선점), **로그인은** 성공하면 `clear` 로
 * 지운다. 정직한 사람에게는 결과가 같고, 동시에 쏟아지는 쪽에게만 다르다.
 *
