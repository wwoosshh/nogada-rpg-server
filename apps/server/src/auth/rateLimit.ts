 ...
 *   무르지 않는다(`clear` 를 부르지 않는다). 실측 기준: 한 IP 가 **연달아 5개**
 *   까지는 자유롭게 열고, 6번째부터 5초 대기가 실패마다 배증해 5분에서 멎는다
 *   (SIGNUP_BACKOFF). 상한이 있으니 쫓아내는 것이 아니라 늦추는 것이다.
 *   가입만 세는 표가 따로 있는 이유는 SIGNUP_BACKOFF 의 주석에 있다.
 *
