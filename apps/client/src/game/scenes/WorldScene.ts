...
export class WorldScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    // 노드 그림도 **이 맵에 놓인 것만** 올린다 — 화자 시트와 같은 이유다. 열두
    // 장은 다 합쳐도 9KB 라 지금은 전부 올려도 티가 안 나지만, 노드 종류는 채집장이
  ...
  private checkTransition(here: TilePos): 'stop' | void {
    ...
    void useGameStore
      ...
      .catch(() => {
        ...
        // 되돌아가 순간이동한 것처럼 보인다.
        //
        // 그리고 **밟기 직전 칸으로 되민다**(결계 설계 §9-앞 14). 잠금만 풀면
        // 거절당한 사람이 결계 칸 **위에** 선 채로 남아, 설계와 성공 기준이
        // "밀려난다"고 적은 그 말이 화면에서 거짓이 된다. 결계 문구가 "지금
        // 63,240" 이라고 말하는 자리에 몸은 이미 벽 안쪽 문턱에 서 있는 셈이라,
        // 통과한 것인지 아닌지도 화면에서 읽히지 않는다.
        //
        // 거절 코드를 가리지 않는 이유: 이 걸음이 무엇이었든 서버는 그 문을
        // 열어 주지 않았고, 무르는 것이 어느 거절에나 옳다. 코드별 갈래는
        // 문구를 짓는 스토어의 몫이고(gameStore.move), 씬이 그 어휘를 한 벌 더
        // 아는 순간 같은 사실이 두 곳에서 각자 늙는다.
        this.mover.stepBack()
