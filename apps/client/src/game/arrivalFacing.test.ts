...
const transitions: TransitionDef[] = [
  ...
  { fromMap: '얼음채집장', fromX: 0, fromY: 5, toMap: '동굴', toX: 9, toY: 5, facing: null },
]

describe('arrivalFacing', () => {
  // 왜: facing 은 파싱되고, 좋은 메시지로 타입 검사까지 받고, GameData 에
  //     실려 나가는데 **아무도 읽지 않았다.** 얼음채집장 (3,0) 에서 북쪽으로 걸어
  //     나가면 눈의마을 (10,13) 에 남쪽을 보고 도착했다 — 방금 나온 전환을
  //     정면으로 마주 보면서. 눈에 보이게 틀린 것이고, 채워 넣어도 아무 일도
  //     안 일어나는 열은 작가에게 더 나쁜 결과다.
  it('도착 칸에 적힌 facing 을 그대로 본다', () => {
    expect(arrivalFacing(transitions, '눈의마을', { x: 10, y: 13 }, 'down')).toBe('up')
    expect(arrivalFacing(transitions, '얼음채집장', { x: 3, y: 1 }, 'up')).toBe('down')
  })
  ...
  it('그 칸에 도착하는 전환이 없으면 들어온 방향을 그대로 쓴다', () => {
    expect(arrivalFacing(transitions, '얼음채집장', { x: 20, y: 20 }, 'right')).toBe('right')
  })

  // 왜: 두 맵이 같은 좌표를 갖는 것은 규칙이 아니라 우연이다. 맵을 안 보면
  //     눈의마을 (3,1) 에 선 사람이 얼음채집장 로 들어온 전환의 방향을 물려받는다.
  it('좌표가 같아도 다른 맵의 전환은 보지 않는다', () => {
    expect(arrivalFacing(transitions, '눈의마을', { x: 3, y: 1 }, 'right')).toBe('right')
  })
})
