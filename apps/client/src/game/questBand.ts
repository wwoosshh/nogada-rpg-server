 ...
 *
 * **`x`·`y` 를 무는 자는 아직 없다** — 지금 자가 쓰는 것은 `width` 와 `padding`
 * 뿐이라 이 둘은 아무 값으로 바꿔도 전부 초록이다. x=131 은 설계 ⑤ 가 미니맵을
 * (9,39) 116×116 에 세우기로 한 데서 나온 「미니맵 오른쪽」이라는 약속이고, 그
 * 약속을 지키는 것이 지금은 사람 눈뿐이다. 자는 **미니맵 태스크(설계 ⑧-7)에서**
 * 두 자리를 함께 물려 세운다 — 「미니맵 오른끝 + 여백 = 띠 왼끝」과 「띠 오른끝 +
 * 여백 = 화면 폭」. 지금 따로 지으면 미니맵 상수가 없어 자가 자기 상상을 재게 된다.
 */
export const BAND = {
  x: 131,
  y: 39,
