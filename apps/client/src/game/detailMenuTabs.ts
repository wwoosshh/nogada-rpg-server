 ...
 *
 * **머리가 세는 것은 아직 안 넘은 것이다.** 한동안 자루에 담긴 전부를 셌는데,
 * 신규는 40개가 전부 미달성이라 두 수가 우연히 같아서 그 어긋남이 안 보였다.
 * 지금 서버에 살아 있는 얼음 200,000 테스터에게는 「그 뒤의 문 N개」의 N 안에
 * **이미 연 문**이 섞인다 — 「그 뒤」라는 이름이 세는 수와 어긋나는 것이다.
 *
 * 그렇다고 넘은 것을 안 적으면 「5개」라고 써 놓고 펼치면 17줄이 나온다. 그래서
 * 넘은 몫은 목록이 이미 쓰는 글자(`✓`)로 뒤에 붙인다 — 같은 자루 안의 달성한
 * 줄들이 머리에 붙는 그 표를 그대로 쓴다.
 */
function foldLines(
  id: string,
  label: string,
  ...
  const left = rows.filter((r) => !r.achieved).length
  const done = rows.length - left
  const lines: ScrollListLine[] = [
    {
      text: `─── ${label} ${left}개${done > 0 ? ` ✓${done}` : ''}   ${open ? '[접기]' : '[펼치기]'}`,
      color: DIM_COLOR,
      fontSize: ROW_NAME_FONT_SIZE,
      groupId: id,
    },
 ...
 * 자루에 그대로 둔다. 다 캔 사람의 화면이 머리 없이 자루 둘만 남는 것도 참말이다
 * — 그에게는 열 문이 정말 없다.
 *
 * **이 규칙이 사는 값은 첫 화면의 장식 줄이 0 이 되는 것이다.** `고르게`를 머리로
 * 세우면 「칭호 — 효과는 없다」가 첫 화면에 한 줄 돌아온다. 이 아크가 고치려던
 * 것이 정확히 그 문구의 개수(열넷이었다)이므로, 하나와 0 은 다른 수다.
 *
 * 설계 ④ 는 일곱 묶음 열네 줄을 그렸고 이 규칙은 그것을 여섯 묶음 열두 줄로
 * 줄인다(설계 ⑧-6 의 통과 기준도 같이 고쳐 뒀다). **줄인 값을 부풀려 적지
 * 않는다:** 일곱 묶음이면 줄이 열여섯 288px 이고 뷰포트는 812×375 에서 255px 라
 * 33px, 약 두 줄이 넘친다. 이 목록은 드래그·휠로 도는 `ScrollList` 이므로 접이
 * 머리 둘은 사라지는 것이 아니라 **한 번 끄는 거리**에 있다. 그 두 줄을 아끼는
 * 것은 덤이고, 이 규칙의 이유가 아니다.
 *
