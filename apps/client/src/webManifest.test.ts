 ...
 * 전부 manifest 쪽 요건이고, 브라우저가 그것을 보고 무엇을 하는지는 못 잰다.
 * 그 답은 실기에서 나왔다: **2026-08-16 안드로이드 크롬에서 ⋮ 메뉴에 `앱 설치`
 * 가 뜨는 것을 확인했다.** 서비스워커 없이 된다.
 *
 * 그 확인이 **이 파일에 원래 적혀 있던 전제를 뒤집었다.** 여기에는 "안드로이드
 * 설치 배너·WebAPK 는 fetch 핸들러를 가진 서비스워커를 함께 요구해 왔는데 이
 * 클라이언트에는 0개다(grep 실측)"라고 적혀 있었다. grep 은 맞았고 **결론이
 * 틀렸다** — 크롬은 그 요건을 놓은 지 오래다. 서비스워커는 붙이지 않는다.
 *
 * 남는 경계는 그대로다: 아래가 전부 초록이라고 폰이 반드시 설치를 제안한다는
 * 뜻은 아니다. manifest 쪽 요건이 갖춰졌다는 뜻일 뿐이고, 브라우저의 판정 규칙이
 * 바뀌는 날 이 자들은 아무 말도 안 한다. 그때는 다시 폰에서 눌러 봐야 한다.
 */
