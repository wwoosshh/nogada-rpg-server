...
- [ ] **Step 1: B 버튼을 상세 메뉴로 만든다**

지금 B 는 열린 패널을 닫기만 한다. **아무것도 열려 있지 않을 때 B 를 누르면 상세 메뉴가 열리도록** 바꾼다. 무언가 열려 있으면 여전히 닫는다 — 휴대폰 뒤로 가기와 같은 규칙이고, B 의 의미가 하나로 유지된다.

상세 메뉴는 탭으로 나뉜다.

| 탭 | 내용 |
|---|---|
| 숙련도 | 다섯 기술의 현재 숙련도. `actionIntervalMs` 로 계산한 행동 간격도 함께 |
| 이정표 | 이 태스크가 만드는 목록 (아래) |
| 설정 | 지금 상단 바 우측 톱니를 눌렀을 때 나오는 것 |

**원작이 정확히 이 구조였다.** 특수 메뉴를 호출하는 커먼이벤트 이름이 `[★B]특수메뉴호출` 이고 숙련도 정보 화면이 그 안에 있었다.

상단 바의 톱니는 **같은 메뉴의 설정 탭을 여는 두 번째 입구**로 남긴다. 지우지 않는다 — 사용자가 그 자리에 두라고 정한 것이고, Task 4 가 그 옆에 한 줄을 넣는다.

이벤트·퀘스트 탭은 나중에 더한다. **탭을 더하는 데 이 파일 밖을 건드릴 필요가 없는 구조로 만든다.**
...
브라우저에서 가로 화면:
1. 아무것도 열려 있지 않을 때 B 를 누르면 상세 메뉴가 열린다
2. 가방 패널이 열려 있을 때 B 를 누르면 메뉴가 아니라 가방이 닫힌다
3. 이정표 탭에 달성한 것과 못 한 것이 전부 보인다
4. 못 한 것에 지금 값과 필요한 값이 숫자로 나온다
5. 목록이 화면보다 길면 스크롤된다
6. 상세 메뉴가 열린 동안 캐릭터가 움직이지 않는다 (기존 게이트가 그대로 동작한다)
7. **조합 200 줄이 "구리 망치를 만들 수 있다" 로 보인다** — 칭호와 다른 줄이 섞여 있다
8. 숙련도 탭에 다섯 기술의 값이 나온다
9. 상단 바의 톱니를 누르면 같은 메뉴의 설정 탭이 열린다
10. 닫으면 조작이 돌아온다

각 항목의 결과를 보고한다. **7번이 이 태스크의 핵심이다** — 목록이 칭호만 나열하면 원작이 한 일의 껍데기만 가져온 것이다. 2번도 중요하다 — B 의 의미가 둘로 갈라지면 조작이 헷갈린다.
