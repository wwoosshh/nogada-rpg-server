...
- [ ] **천장은 상대, 바닥은 절대다. 둘을 같은 자로 재면 안 된다.**
      - **천장 `SPECIAL_YIELD_MAX`(상대)**: 특수 표의 모든 구간이 같은 숙련·같은 손에서
        그 계열 바깥 표의 같은 구간보다 **낮다**(비 < 1.0). ∞ 브라켓 포함.
      - **바닥 `SPECIAL_YIELD_MIN_STEP`(절대)**: 특수 표 **최상위 티어의 확률**이
        브라켓마다 **최소 ×1.2 오른다.** 비의 하한이 아니다.

      **바닥의 자를 두 번 고쳤다(B4 에서 실측으로).** 분당 골드로 재면 `cum1` 을
      브라켓마다 똑같이 눕힌 표도 500ms 가 297ms 가 되는 것만으로 ×1.68 이 되어
      통과한다 — 표가 아니라 숙련도가 오른 것이다. 회당 기대 매도가로 옮기면
      **잡티어가 값을 지배해** §5 목표표가 17.6 → 19.0 → 20.1 → 23.9G 로 ×1.1 씩만
      움직인다. 특수 표가 파는 것은 골드가 아니라 **그 한 아이템**이므로 바닥은
      최상위 티어의 확률을 죈다.

      **왜 바닥이 비가 아닌가 — 재 봤다.** §5 목표를 만족하는 `ice_special` 후보
      (뜨거운 얼음 정가 12,000 · 잡티어 `ice_shard` · 성공률은 바깥과 동일)를
      구리손·브라켓 하한에서 재면:

      | 브라켓(하한 숙련) | `cum1` | 뜨거운 얼음 1개까지 | 비(특수 ÷ 바깥 G/분) |
      |---|---|---|---|
      | ≤500 (0) | 20 | 39.7분 | 0.65 |
      | ≤5,000 (501) | 25 | 19.0분 | 0.54 |
      | ≤10,000 (5,001) | 31 | 11.6분 | 0.48 |
      | ≤150,000 (10,001) | 38 | 8.5분 | 0.34 |
      | ≤500,000 (150,001) | 46 | 4.0분 | 0.13 |
      | ∞ (500,001) | 61 | 2.0분 | **0.012** |

      **`roll <= cum` 이 포함이라 확률은 `(cum+1)/100001` 이다.** 그 +1 을 안 세면
      20 → 24 가 ×1.19 로 바닥에 1분 모자란다. B7 이 이 사다리를 그대로 쓴다.

      **바깥 ∞ 가 한 칸에 ×11.4 로 터지므로**(`ice_gem` 이 15%) 비의 하한을 어디에
      두든 마지막 계단이 ×0.09 로 떨어진다. 비를 0.1 위로 유지하려면 뜨거운 얼음의
      정가가 **67만**이 되어야 하고, 그것은 "특수 노드는 골드가 아니라 열쇠를 판다"를
      정면으로 뒤집는다. **그러나 특수 표 자신의 진행은 단조다**(1,958 → 3,560 →
      5,062 → 6,769 → 14,227 → 23,543, 계단마다 평균 ×1.7). 바닥은 그것을 문다.
- [ ] **∞ 브라켓은 한 숫자가 아니다.** 같은 표·같은 손으로 숙련 500,001 에서
      1,679,054, 1,000,000 에서 2,451,419 다(간격만 계속 준다). **∞ 를 잴 때는
      브라켓의 하한(500,001)에서 잰다** — 구간 안에서 간격은 줄기만 하므로 하한이
      최악이고, 어느 한 점을 안 정하면 그 검사는 참·거짓을 물을 수 없다.
