...
| # | 문제 | 위치 |
|---|---|---|
| 1 | 요청 로그가 꺼져 있다 → 공격 흔적 0 | `config.ts:93`, WinSW XML |
| 2 | 500 응답이 내부 오류 message 를 뱉는다(호스트·포트·errno) | `app.ts:106` |
| 3 | 평문 HTTP 위의 30일 베어러 토큰 | `index.ts:22` |
| 4 | 인증 없는 `POST /api/auth/logout` 이 무제한 DB 왕복 | `app.ts:135` |
| 5 | `CORS_ORIGIN` 에 공개 오리진 없음 | 서버 `.env` |

**1번은 어떤 코드 수정보다 먼저다.** `LOG_LEVEL` 도 `NODE_ENV` 도 배포에 없어
`parseLogger` 가 false 를 돌려주고, WinSW 서비스는 요청 로그를 한 줄도 안 남긴다.

**5번**은 같은 오리진 서빙을 택하면 웹에서는 CORS 자체가 발생하지 않는다. APK 를
