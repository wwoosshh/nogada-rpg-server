...
| # | 문제 | 위치 |
|---|---|---|
| 1 | **가입 성공이 레이트리밋에 안 세진다** → 한 IP 에서 초당 211계정 | `routes/auth.ts:44-55` |
| 2 | 만료 세션 청소 없음 → 세션 행 영구 누적 | `auth/sessions.ts:101-106` |
| 3 | 요청 로그가 꺼져 있다 → 공격 흔적 0 | `config.ts:93`, WinSW XML |
| 4 | 500 응답이 내부 오류 message 를 뱉는다(호스트·포트·errno) | `app.ts:106` |
| 5 | 평문 HTTP 위의 30일 베어러 토큰 | `index.ts:22` |
| 6 | 인증 없는 `POST /api/auth/logout` 이 무제한 DB 왕복 | `app.ts:135` |
| 7 | `CORS_ORIGIN` 에 공개 오리진 없음 | 서버 `.env` |

**1번이 가장 급하다.** `byIp.fail()` 이 아이디 중복(409) 경로에서만 불린다 —
성공(201)은 안 센다. 실측: 같은 IP 순차 60회에 429 하나도 없고, 동시 128회는
606ms 에 128개 전부 201. 고치는 법은 성공 경로에도 `byIp.fail(ipOf(request), now)`
를 부르는 것 — **새 의존성 불필요**(`@fastify/rate-limit` 을 깔 필요 없다).

단 그것만으로는 부족하다. 게이트가 `retryAfterMs() → await(argon2) → fail()`
구조라 **동시 버스트는 첫 라운드가 전부 통과한다.** 로그인도 같다(직렬 7번째에
429, 동시 64회는 401 이 64개). 리미터가 세는 것은 "추측 횟수"가 아니라 "왕복 라운드
수"다. `await` **앞에서** 예약하는 카운터를 함께 넣고, 회귀 테스트는 **동시 N회**로
쓴다.

**3번은 어떤 코드 수정보다 먼저다.** `LOG_LEVEL` 도 `NODE_ENV` 도 배포에 없어
`parseLogger` 가 false 를 돌려주고, WinSW 서비스는 요청 로그를 한 줄도 안 남긴다.

**7번**은 같은 오리진 서빙을 택하면 웹에서는 CORS 자체가 발생하지 않는다. APK 를
...
- **게임 행동 API 에 레이트리밋 없음.** 요청당 0.1ms(7,700~14,000 req/s)이고
  쿨다운·난수·판정이 전부 서버라 연타로 부정 이득이 없다. 2장 1번을 막으면 이
  항목의 위험은 크게 준다.
...
- **CORS 가 넓은 것 자체.** 인증이 쿠키가 아니라 Authorization 헤더라 CSRF 도 토큰
  탈취도 성립하지 않는다. 다만 남의 페이지가 방문자 브라우저로 가입을 두드려 IP
  카운터를 흩을 수 있어 2장 1번과 세트로 좁히는 것이 맞다.
...
`apps/server/src/index.ts:22` 의 `host: '0.0.0.0'` 은 하드코딩이라 환경변수로 못
바꾼다. `host: process.env.HOST ?? '0.0.0.0'` 으로 빼고 `HOST=127.0.0.1` 을 주는
것이 맞다. **안 고치면 터널을 세워도 3000 이 LAN·Tailscale 에 평문으로 계속 열려
있다.**
