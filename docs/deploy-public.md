...
발생하지 않는다**(실측: 브라우저가 프리플라이트를 한 건도 안 보냈다). 그래서 이제
운영에서 빈 `CORS_ORIGIN` 은 전부 허용이 아니라 **전부 거절**이고, 3단계가 그 줄을
빈 채로 적게 한다(아래 "이미 닫힌 것"). 남는 것은 APK 뿐이고, 붙일 때
`https://localhost` 를 추가한다 — `capacitor://localhost` 가 아니다.

### 이미 닫힌 것 (다시 하지 마라)

위 표에 두 줄로 있던 인증 관문이 **`b44f582` 에서 닫혔고**, 그 뒤 `dbd4d27` 이
노출 표면 쪽 둘을, 이 문서를 고치면서 CORS 하나를 덧붙였다 — 한 줄이 실제로는
둘이었으므로 아래는 **여섯**이다.
지운 자리에 이것을 남기는 이유는 같은 실측을 다시 하지 않게 하려는 것이고, 닫힌
것을 아무도 되돌리지 않게 하려는 것이다. 여섯 다 회귀 검사가 있고 그중 둘은
**동시 N회**로 쓰였다 — 순차로 쓰면 그 버그가 다시 들어와도 초록이라, 그것이
이 버그가 여태 살아남은 이유였다.
...
- **가입 성공이 레이트리밋에 안 세지던 것.** 실측이었던 "한 IP 에서 초당 211계정"
  ...
  한 IP 가 연달아 **6개까지** 열고 **7번째가** 429 를 받으며, 그 뒤로 5초 대기가
  하나 열 때마다 배증해 5분에서 멎는다. 자유 횟수는 5 인데 6개가 열리는 것이
  맞다 — 문을 **지나고 나서** 세기 때문이다(`routes/auth.ts` 가 `retryAfterMs` 를
  먼저 보고 그 뒤에 `fail` 한다). 실측:
  `201 201 201 201 201 201 429 429 429`, `retry-after: 5`.
  (오래 "5개까지, 6번째부터"라고 적혀 있었다. 한 칸 틀렸고, 이 문장이 "공개해도
  되는가"의 근거로 쓰이는 자리라 고쳐 둔다 — 이제 `auth.test.ts` 의 '가입은
  성공해도 세어진다' 가 그 경계를 못 박는다.)
...
- **빈 `CORS_ORIGIN` 이 "전부 허용"이던 것.** 이 문서는 "웹에는 필요 없다"고
  적었고 그건 사실인데(같은 오리진), 그래서 그 줄이 없는 배포는 **아무 오리진에나
  허용 헤더를 돌려줬다** — 실측: `Origin: https://evil.example` 에 그대로
  `access-control-allow-origin: https://evil.example`, 프리플라이트 204.
  이제 운영에서 빈 값은 전부 거절이고, 개발 콘솔에서만 예전처럼 전부 허용이다
  (판정은 위 둘과 **같은 함수**다 — `config.ts` 의 parseCorsOrigin).
  같은 오리진 서빙은 CORS 를 안 쓰므로 웹은 안 깨진다.
  회귀 검사는 `config.test.ts` 의 `CORS 배선` 이고 **음성 대조군까지** 잰다 —
  허용 오리진만 재면 "전부 허용"과 "제대로 좁혀짐"을 구별할 수 없다.
...
- **CORS 를 좁힌 것이 방어의 전부는 아니라는 것.** 2장에서 닫았지만, 브라우저
  밖에서 오는 것(curl·스크립트)에는 CORS 가 애초에 아무 일도 안 한다 — 그쪽을
  막는 것은 레이트리밋 하나다. 인증이 쿠키가 아니라 Authorization 헤더라 CSRF 도
  토큰 탈취도 성립하지 않으므로, CORS 로 얻은 것은 "남의 페이지가 방문자
  브라우저를 빌려 가입을 두드리는 길"이 막힌 것까지다.
...
**`CORS_ORIGIN` 은 값 없이, 그러나 적는다.** 같은 오리진 서빙이라 웹은 교차 출처
요청을 아예 안 만들고(실측: 프리플라이트 0건), 운영에서 빈 값은 **"전부 허용"이
아니라 "아무 데도 허용 안 함"**이다. 그 갈림은 요청 로그·500 리댁션과 **같은
판정**을 쓴다(`config.ts` 의 isDevConsole): 사람이 앞에 앉은 개발 콘솔에서는 빈 값이
지금까지처럼 전부 허용이고(Vite dev 서버가 5173 에서 붙는다), 서비스·컨테이너처럼
아무도 안 보는 자리에서는 전부 거절이다. 여기서 `NODE_ENV` 하나로 갈랐으면
**WinSW 가 그 변수를 안 놓아 배포가 개발인 척하며 열린 채로 섰을 것이다** —
요청 로그가 여태 안 남던 것과 같은 함정이다(2장 "이미 닫힌 것").

줄을 지우지 말고 남기는 이유는 **APK 를 붙이는 날 손댈 자리가 여기**이기
때문이다. 그때 적을 값은 `https://localhost` 다 — `capacitor://localhost` 가
아니다(5장 5번).

좁힐 값어치가 얼마인지는 정직하게 적어 둔다: 토큰이 쿠키가 아니라 localStorage 라
고전적 CSRF 는 성립하지 않는다(3장). 남는 것은 **아무 사이트나 방문자의 브라우저로
`/api/auth/login`·`register` 를 두드릴 수 있다**는 것이고, 그때 IP 백오프의 열쇠가
방문자 IP 로 흩어져 무력해진다. 치명은 아니고, 닫는 값이 0원이라 닫는다.

`CLIENT_DIST` 는 안 적는다 — 기본값이 이 저장소의 `apps/client/dist` 이고,
6단계가 dist 를 밀어 넣는 자리가 바로 거기다.
...
문제를 쫓느라 서비스를 멈추고 `deploy-windows.md` 3장의
...
CORS 가 실제로 닫혔는지. **음성 대조군이다** — 허용 오리진만 테스트하면 "전부
허용"과 "제대로 좁혀짐"을 구별할 수 없다. 그리고 3단계대로라면 허용 오리진이
아예 없는 것이 정상이므로(웹은 CORS 를 안 쓴다), 여기서 재는 것은 "좁혀졌는가"가
아니라 **"닫혔는가"**다:
...
기대 출력 — `200` 이고 **`access-control-allow-origin` 도 `vary` 도 없다.** 서버
자신이 내는 줄은 이 셋뿐이다(실측, 터널을 지나면 Cloudflare 가 몇 줄 더 붙인다):

```
content-type: application/json; charset=utf-8
x-server-now: 1786830442382
content-length: 58
```

**보는 것은 있는 줄이 아니라 없는 줄이다.** 눈으로 세지 말고 이렇게 묻는다 —
아무것도 안 나오는 것이 통과다:

```bash
curl -is -H "Origin: http://evil.example" https://nogada.내도메인/api/health \
  | grep -i "access-control-allow-origin\|^vary"
```

`access-control-allow-origin: http://evil.example` 가 **돌아오면 열려 있는
것**이다. 원인은 셋 중 하나다: ① `.env` 에 `NODE_ENV=production` 이 없는 채로
콘솔에서 띄웠다(그러면 개발로 잡힌다 — 3단계), ② `CORS_ORIGIN` 에 `*` 가 들어
있다, ③ `.env` 를 고치고 서비스를 다시 안 띄웠다(`.env` 는 기동 때 한 번만 읽힌다).

`vary: Origin` 만 나오고 허용 헤더는 없다면 **목록이 설정돼 있고 이 오리진이 그
목록 밖**이라는 뜻이다 — APK 를 붙인 뒤(`CORS_ORIGIN=https://localhost`)라면 그것이
정상이다.

프리플라이트도 함께 본다. 닫혀 있으면 **204 가 아니라 404** 다:

```bash
curl -i -X OPTIONS -H "Origin: http://evil.example" \
  -H "Access-Control-Request-Method: POST" https://nogada.내도메인/api/auth/login
```

(이 둘은 `apps/server/src/config.test.ts` 의 `CORS 배선` 이 양성·음성 대조군을
함께 재는 것과 같은 자리다. 서버 PC 에서 재는 것은 그 자가 **실제 배포의 `.env`**
까지는 못 보기 때문이다.)

레이트리밋이 붙었는지 — 가입을 20번 연달아 시도해 429 가 나오는지 본다. **6개까지
201 이고 7번째부터 429**다(2장). 그리고 로그의 remoteAddress 가 127.0.0.1 이 아닌
실제 IP 인지 눈으로 확인한다.
...
**설치는 실제로 된다 — 실기 확인 완료(2026-08-16, 안드로이드 크롬).** ⋮ 메뉴에
`앱 설치` 가 뜨는 것을 사용자가 눈으로 확인했다.

**이것이 뒤집은 전제 하나를 적어 둔다.** 이 문단은 원래 "안드로이드 설치
배너·WebAPK 는 fetch 핸들러를 가진 **서비스워커**를 함께 요구해 왔는데 이
클라이언트에는 서비스워커가 0개라 아마 안 뜰 것이다"라고 적혀 있었다. **틀렸다.**
크롬은 그 요건을 놓은 지 오래고, 지금 이 사이트는 HTTPS · manifest(name·icons·
start_url·display) · 192/512 아이콘만으로 설치 제안을 받는다. **서비스워커는 안
붙인다** — 없이 되는 것을 확인했고, 오프라인 재생이 필요해지기 전에는 캐시 무효화
문제만 사 오는 물건이다(dist 를 사람이 손으로 미는 이 배포에서는 특히).

아이폰은 별개다 — 사파리는 `display`·`orientation` 을 무시하고 `apple-touch-icon`
한 줄로만 아이콘을 정한다. 아이폰 사용자에게 가로를 지키는 것은 이 문서 아래의
세로 안내(`@media (orientation: portrait) and (max-width: 811px)`)뿐이다.

> **실기 확인을 남에게 부탁할 때** — "⋮ 에 앱 설치가 뜨는지 보세요"는 안내가
> 아니다. 이 건에서 실제로 두 번 왕복했다. 부탁에는 (1) 그게 뭔지 한 줄
> (2) 시작 상태(기기·앱·주소를 직접 칠 것) (3) **누를 곳의 위치**("오른쪽 위 끝의
> 점 세 개, 주소창과 같은 줄") (4) **볼 것과 그 뜻을 갈라 놓은 표**
> (5) 돌려줄 말의 형식을 다 적는다.
...
**세로로 들면 패널이 무너지던 것 — 안내를 세웠다.** 마을 카드는 `repeat(4, 1fr)`,
제작·상점은 좌우 분할이라 375px 폭에서 못 쓰는데 **캔버스는 멀쩡히 늘어나서**
"게임은 떴는데 창을 열면 못 쓴다"로 나타났다. manifest 의 `orientation: landscape`
로는 안 끝난다 — 그건 홈 화면에 추가해 standalone 으로 열었을 때만 걸리고, 친구가
링크를 처음 여는 방식은 그냥 브라우저이며, **iOS 사파리는 그 칸을 아예 안 읽는다.**

지금은 `ui.css` 의 `@media (orientation: portrait) and (max-width: 811px)` 한 겹이
화면을 통째로 덮고 가로로 돌려 달라고 말한다(문구는 `ui/OrientationNotice.tsx` 의
상수 한 벌이 전부다 — 여기에 따옴표로 옮겨 적으면 그것이 두 번째 사본이 되고,
`ALREADY_FULL_TEXT` 가 그 교훈이라 일부러 풀어 쓴다).
**레이아웃을 반응형으로 고친 것이 아니다** — 이 게임은 가로 전용이고 그대로 둔다.

둘을 실측으로 못 박아 둔다(위 문단대로 `@fastify/static` 경로에서 쟀다):

- **폭 조건이 함께 걸린 이유.** `orientation: portrait` 는 기기 방향이 아니라
  뷰포트 종횡비라 세로로 긴 데스크톱 창(900×1200)도 걸린다 — 거기서 "돌리라"는
  말은 할 수 없는 일을 시키는 것이다. 실제로 무너지는 원인은 방향이 아니라 폭이다.
  경계는 설계 폭 812 이고 실측했다: **812×900 은 안 뜨고 811×900 은 뜬다.**
  (방향 조건을 빼고 폭만 재도 안 된다 — 아이폰 SE 를 가로로 든 667×375 가 811
  아래라, 이미 가로인 사람에게 가로로 돌리라고 말하게 된다.)
- **덮개일 뿐 언마운트가 아니다.** 게임에 들어간 상태에서 375×812 로 바꿨다가
  812×375 로 되돌려 마을·HP·골드·캔버스 크기가 **전부 같음**을 확인했다. 방향을
  아는 자는 CSS 하나뿐이라(스토어도 훅도 안 만들었다) 돌리는 순간 리렌더도 없다.

**남는 자리 — 812 미만 가로 화면은 아무 말도 안 듣는다.** 아이폰 SE·8 을 가로로
든 667×375 가 그 구간이고, 이 안내가 "옆으로 눕히세요"로 사람을 보내는 목적지이기도
하다. 검토가 여기서 "캔버스가 812 고정이라 오른쪽 145px 이 잘린다"고 적었는데
**재 보니 그렇지 않았다** — 다음 사람이 그 숫자를 믿지 않도록 실측을 남긴다:

- 캔버스는 폭을 따라 줄어든다. `PhaserGame.ts` 가 `Scale.NONE` 으로 띄우고
  ResizeObserver 가 부모 `clientWidth` 로 다시 잡는다(`parentSize`). 게임 코드에
  812 라는 **상수는 없다**(그 숫자가 있는 자리는 전부 주석이다). 잘리는 것이
  아니라 세계가 덜 보인다.
- DOM UI 도 유동이다. 667×375 게이트 화면에서 `#root` 의 scrollWidth 와
  clientWidth 가 **둘 다 667** 이다 — 가로로 넘치는 것이 없다.

그래서 이 구간은 "못 쓰는 화면"이 아니라 "좁은 화면"이고, 무엇보다 **그 사람에게
시킬 행동이 없다**(이미 가로다). 침묵을 지금은 그대로 둔다. 문구가 필요해지는 날이
오면 "돌리세요"가 아니라 "화면이 좁아 일부만 보입니다" 쪽이고, 근본 해법은 캔버스를
폭에 맞춰 축소하는 것이다. **아직 안 잰 것:** 플레이 중 화면은 계정이 있어야 들어가
못 쟀다 — 667 폭에서 제작·상점 좌우 분할이 실제로 쓸 만한지는 아무도 안 봤다.

> **헤드리스 크롬으로 스크린샷을 찍어 확인하려거든 뷰포트를 못 박아라.**
> `--window-size=375,812` 는 **창** 크기이고 레이아웃 뷰포트가 그것과 같다는 보장이
> 없다 — 실제로 안 같았다. 안내 글이 오른쪽으로 잘려 나온 그림을 한 번 믿고
> 없는 줄바꿈 버그를 쫓았다. CDP 의 `Emulation.setDeviceMetricsOverride` 로 폭을
> 지정하면 실제 브라우저와 같은 그림이 나온다. **숫자는 그림이 아니라
> `getBoundingClientRect`·`getComputedStyle` 로 읽는다.**

회귀는 `apps/client/src/ui/orientationNotice.test.ts` 가 잡는다. 다만 그 자가 재는
것은 소스뿐이다 — **진짜로 덮이는지는 브라우저에서만 보인다**(위 실측이 그것이다).
