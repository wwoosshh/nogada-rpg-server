...
| # | 문제 | 위치 |
|---|---|---|
| 1 | 평문 HTTP 위의 30일 베어러 토큰 | 터널이 서기 전까지 |
| 2 | 인증 없는 `POST /api/auth/logout` 이 무제한 DB 왕복 | `app.ts` 의 auth 라우트 |
...
(2장 "이미 닫힌 것"). 적는 것은 조용히 하고 싶을 때(`warn`)뿐이다.

이 두 줄을 넣었으면 서비스를 다시 띄우고(`nogada-server.exe restart`) 로그 파일에
