...
### `CORS_ORIGIN` — 웹만 열 거면 **빈 채로** 적는다

서버가 게임 화면을 **API 와 같은 오리진으로** 내주므로(7장의 dist) 브라우저는
교차 출처 요청을 아예 안 만든다(실측: 프리플라이트 0건).

**운영에서 빈 값은 "전부 허용"이 아니라 "아무 데도 허용 안 함"이다.** 갈림은
바로 위 `NODE_ENV` 와 **같은 판정**을 쓴다(`config.ts` 의 isDevConsole) — 콘솔에서
손으로 띄운 서버는 지금까지처럼 전부 허용이고(개발 PC 의 Vite dev 서버가 그것에
기댄다), 서비스로 도는 서버는 전부 거절이다. 그러니 `.env` 에 `CORS_ORIGIN=`
한 줄을 남겨 두는 것이 값을 지우는 것보다 낫다: 결과는 같고, APK 를 붙이는 날
손댈 자리가 눈에 남는다.
